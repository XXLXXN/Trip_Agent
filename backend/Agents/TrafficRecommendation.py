...
base_payload = {
    ...
    "bot_app_key": "xndVuDryhkzSiHfYbIzOAdXwXhCmTPAVqXfDHAoAIhsPwuzsvUAkeQelYquVynBwXTxVwMSBoXCQbTBIvaHtUDknNuAlSKREQbKKnQZfIjIxYVgAvTdyrknlXjmmHPWz",
...
# --- 调试专用的核心功能函数 ---
async def get_traffic_recommendation(content: str) -> list:
    """
...
async def main():
    ...
    """
    # 在这里定义你想发送的内容
    user_input = """  我计划10月15号从成都去丽江玩，行程名称是“成都-丽江双城记”。大概在丽江待5天左右，然后返回成都。
                去程我想体验一下火车，看看沿途风景，时间长点没关系。返程因为第二天要上班，所以希望选择快一点的飞机。麻烦你帮我推荐一下性价比高的去程和返程票务方案。"
            """
    print(spot_api_key)
    # 调用核心函数并等待结果
    final_replies = await get_traffic_recommendation(user_input)
