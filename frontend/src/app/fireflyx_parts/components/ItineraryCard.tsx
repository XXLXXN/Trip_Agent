...
export const ItineraryCard: React.FC<ItineraryCardProps> = ({
  ...
  if (type === "transport") {
    ...
    return (
      <>
        <TransportCard 
          className={`cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-[1.02] ${isExpanded ? 'min-h-[120px]' : ''}`}
          onClick={() => {
            console.log('Transport card clicked, current expanded:', isExpanded);
            ...
            <div className="flex-1 grid gap-2">
              ...
              <div className="grid gap-1 text-[10px] text-[#808080] uppercase font-medium" style={{ fontFamily: 'Inter', letterSpacing: '0.8px' }}>
                ...
                {transportData.distance && (
                  <div className="flex items-center">
                    <span className="inline-flex items-center gap-1">
                      <span className="w-3 h-3 rounded-full bg-[#0768FD]"></span>
                      {transportData.distance}
                    </span>
                  </div>
                )}
              ...
              {isExpanded && (
                <div className="mt-3 pt-3 border-t border-[rgba(0,0,0,0.06)]">
                  <div className="grid gap-2">
                    {transportData.cost !== undefined && (
                      <div className="flex items-center justify-between text-[14px]">
                        <span className="text-[#808080]" style={{ fontFamily: 'Inter' }}>费用</span>
                        <span className="text-[#0768FD] font-semibold" style={{ fontFamily: 'Inter' }}>
                          ¥{transportData.cost.toFixed(1)}
                        </span>
                      </div>
                    )}
                    {transportData.description && (
                      <div className="text-[12px] text-[#808080] leading-[16px]" style={{ fontFamily: 'Inter' }}>
                        {transportData.description}
                      </div>
                    )}
                  </div>
